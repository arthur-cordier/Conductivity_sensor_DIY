...
uint16_t adc4_raw_samples[FFT_SAMPLE_SIZE * 2];     // Intercalé IN3/IN4
float32_t adc4_in_f32[FFT_SAMPLE_SIZE];     // Entrée capteur
float32_t adc4_out_f32[FFT_SAMPLE_SIZE];    // Sortie capteur
float32_t fft_output_in[FFT_SAMPLE_SIZE];
float32_t fft_output_out[FFT_SAMPLE_SIZE];
float32_t fft_amplitude_in[FFT_SAMPLE_SIZE / 2];
float32_t fft_amplitude_out[FFT_SAMPLE_SIZE / 2];
/* USER CODE END 0 */
...
{
    HAL_ADC_Stop(&hadc4); // Sécurisation du contexte
    HAL_ADCEx_Calibration_Start(&hadc4, ADC_CALIB_OFFSET, ADC_SINGLE_ENDED);
    HAL_ADC_Start(&hadc4);
    HAL_ADC_PollForConversion(&hadc4, HAL_MAX_DELAY);
    uint16_t raw = HAL_ADC_GetValue(&hadc4);
    HAL_ADC_Stop(&hadc4);

    float voltage = (raw * V_REF) / 4096.0f; // 12-bit resolution
    printf("ADC4 brut : %u, Tension : %.2f V\r\n", raw, voltage);
    return voltage;
}
